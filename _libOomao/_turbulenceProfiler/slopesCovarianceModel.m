...
classdef slopesCovarianceModel < handle
    ...
    methods
        ...
        function obj = slopesCovarianceModel(tel,atm,wfs,gs,varargin)
            ...
            inputs.addParameter('wfsType',[], @isnumeric );
            ...
            obj.gs       = gs;
            %Parameters
            obj.wfsType = inputs.Results.wfsType;
            if isempty(obj.wfsType)
                obj.wfsType = ones(1,numel(gs));
            end
        ...
        function out = manageTipTilt(obj,covMat)
            
            out = covMat;
            % Get filtering matrices
            if any(obj.wfsType == 2) || any(obj.wfsType == 3)
                n = obj.ns;
                rmTT  = obj.tiptiltFilter();
                keepTT= eye(obj.ns) - rmTT;
                ...
                for iGs = 1:obj.nwfs
                    ...
                    for jGs = 1:iGs
                        ...
                        switch obj.wfsType(jGs)
                            case 0
                                out(ri,rj) = 0*tmp;
                            case 1
                                out(ri,rj) = tmp;
                            case 2
                                out(ri,rj) = tmpij*rmTT';
                            case 3
                                out(ri,rj)  = tmpij*keepTT';
                        end
